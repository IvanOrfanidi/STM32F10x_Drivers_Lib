 ...
 */
void Clock::enable(const GPIO_TypeDef* const port) const
{
	if(port == GPIOA) {
 ...
 * @param [in] port - I/O port
 */
void Clock::disable(const GPIO_TypeDef* const port) const
{
	if(port == GPIOA) {
		RCC->APB2ENR &= ~RCC_APB2ENR_IOPAEN;
	} else if(port == GPIOB) {
 ...
 */
void Clock::enable(const SPI_TypeDef* const spi) const
{
	if(SPI1 == spi) {
 ...
 * @param [in] spi - num spi
 */
void Clock::disable(const SPI_TypeDef* const spi) const
{
	if(SPI1 == spi) {
		RCC->APB2ENR &= ~RCC_APB2ENR_SPI1EN;
	} else if(SPI2 == spi) {
...
void Clock::enable(const I2C_TypeDef* const i2c) const
{
	if(i2c == I2C1) {
...
void Clock::disable(const I2C_TypeDef* const i2c) const
{
	if(i2c == I2C1) {
		RCC->APB1ENR &= ~RCC_APB1ENR_I2C1EN;
	} else if(i2c == I2C2) {
 ...
 * @param [in] spi - num spi
 */
void Clock::reset(const SPI_TypeDef* spi) const
{
	if(SPI1 == spi) {
		...
		static_assert("Not correct SPI");
	}
}

/**
 * @brief Reset APB I2C
 * @param [in] i2c - num i2c
 */
void Clock::reset(const I2C_TypeDef* const i2c) const
{
	if(i2c == I2C1) {
