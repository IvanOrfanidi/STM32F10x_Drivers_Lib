...
bool Spi::_isCreated[SpiInterface::MAX_COUNT_SPI];
...
Spi& Spi::getInstance(SPI_TypeDef* const spi)
{
	if(SPI1 == spi) {
		static Spi instance;
		if(!_isCreated[SpiInterface::_1]) {
			instance.init(spi);
			_isCreated[SpiInterface::_1] = true;
		}
		return instance;
	} else if(SPI2 == spi) {
		static Spi instance;
		if(!_isCreated[SpiInterface::_2]) {
			instance.init(spi);
			_isCreated[SpiInterface::_2] = true;
		}
		return instance;
	}
#ifdef STM32F10X_HD
		...
		static Spi instance;
		if(!_isCreated[SpiInterface::_3]) {
			instance.init(spi);
			_isCreated[SpiInterface::_3] = true;
		}
		return instance;
	}
#endif
...
Spi::~Spi()
{
	...
	if(SPI1 == _spi) {
		_isCreated[SpiInterface::_1] = false;
	} else if(SPI2 == _spi) {
		_isCreated[SpiInterface::_2] = false;
	}
#ifdef STM32F10X_HD
	else if(SPI3 == _spi) {
		_isCreated[SpiInterface::_3] = false;
	}
#endif
	else {
	}
